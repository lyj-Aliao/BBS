function preLogin(){
    if ($('#user')[0].value.length == 0 || $('#pw')[0].value.length == 0 || $('#testCode')[0].value.length == 0) {
        alert("账号、密码和验证码必填，请重新输入");
        return false;
    }
    return true;
}
...
var key = 0;
function preModifyPW(){
    if ($("#username")[0].value.length>0 && $("#password")[0].value.length>0 && $("#captcha")[0].value.length>0 && $("#againPW")[0].value.length>0 && $("#emailAddress")[0].value.length>0) {
        if (key) {
...
function reClick(){
    ...
    document.getElementById("sendCode").value="10 秒后可发送";
    x=10;
...
function go(){
    x--;
    if(x>0){
        document.getElementById("sendCode").value=x+" 秒后可发送";
    }else{
